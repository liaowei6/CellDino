            ...
            if "instances" in batched_inputs[0]:
                ...
                if "live_cell" in self.data_loader:
                    ...
                    if self.crop:
                        ...
                        targets = self.prepare_targets_detr(gt_instances, images) #对边框进行归一化，角度没有进行归一化
                else:
            ...
            for mask_cls_result, mask_pred_result, mask_box_result, input_per_image, image_size in zip(
                ...
                if self.instance_on:
                    ...
                    if self.duplicate:
                        ...
                        #instance_r = retry_if_cuda_oom(self.instance_inference)(item_output["pred_logits"][i], item_output["pred_masks"][i], item_box_result)     
                    else:
                        #instance_r = retry_if_cuda_oom(self.instance_inference)(mask_cls_result, mask_pred_result, mask_box_result)
                        instance_r = retry_if_cuda_oom(self.instance_inference_nms)(mask_cls_result, mask_pred_result, mask_box_result)
                        #instance_r = retry_if_cuda_oom(self.instance_inference_cell)(mask_cls_result, mask_pred_result, mask_box_result)
                        #instance_r = retry_if_cuda_oom(self.instance_inference_nms)(item_output["pred_logits"][i], item_output["pred_masks"][i], item_box_result)     
            ...
            new_targets.append(
                {
                    ...
                    "masks": padded_masks,
                    "boxes": targets_per_image.gt_boxes.tensor,
                    "mask_pos": targets_per_image.gt_classes,
                }
            )
        return new_targets
