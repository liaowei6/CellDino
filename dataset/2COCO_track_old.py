...
class tococo(object):
    ...
    def npz_to_coco_val(self):
        ...
        for path in self.data_path:
            ...
            #for dir in sub_dirs:
                ...
                boxes_order = [
                    (box_file.split(".")[0].split("_")[0], box_file)
                    for box_file in os.listdir(path + dir + "/bounding_boxes")
                    ]
                instance_masks_order = [
                    (instance_mask_file.split(".")[0].split("_")[0], instance_mask_file)
    ...
    def npz_to_coco_train(self):
        ...
        for path in self.data_path:
            ...
            for dir in sub_dirs:
                ...
                for i in range(length):
                    #首张图片
                    track_intervals = min(i + 5, length -1)
                    image_path = path + dir + "/images/" + images[i][1]
                    image = tifffile.imread(image_path)
                    h, w = image.shape
                    mask = masks[i][1]
    ...
    def save_json(self):
        ...
        f = open(os.path.join(self.save_path + 'instances_saw.json'), 'w+')
