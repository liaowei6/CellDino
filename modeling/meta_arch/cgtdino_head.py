...
class CgtDINOHead(nn.Module):
    ...
    def forward(self, features, mask=None,targets=None,tracker=None):
        return self.layers(features, mask,targets=targets,tracker=tracker)

    def layers(self, features, mask=None,targets=None,tracker=None):
        mask_features, transformer_encoder_features, multi_scale_features = self.pixel_decoder.forward_features(features, mask)

        predictions = self.predictor(multi_scale_features, mask_features, mask, targets=targets, tracker=tracker)
