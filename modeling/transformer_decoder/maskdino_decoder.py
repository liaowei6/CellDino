    ...
    def prepare_for_dn(self, targets, tgt, refpoint_emb, batch_size):
        ...
        if self.training:
            ...
            if noise_scale > 0:
                ...
                # known_bbox_expand += torch.mul((torch.rand_like(known_bbox_expand)),
                #                                diff).cuda() * noise_scale
    ...
    def forward(self, x, mask_features, masks, targets=None):
        ...
        if self.two_stage:
            ...
            if self.initialize_box_type != 'no':
                ...
                refpoint_embed = inverse_sigmoid(refpoint_embed)  # 后面有sigmoid
        ...
        # iteratively box prediction look-forward twice ?但是在deform detr decoder中output没有进行norm，之后进行的update中output norm了。
        if self.initial_pred:
